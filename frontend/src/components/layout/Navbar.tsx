...
export default function Navbar({ onToggleSidebar, isMobile }: NavbarProps) {
  ...
  return (
    <nav className="bg-eaw-dark text-white h-14 flex items-center px-4 shadow-md z-50">
      <button onClick={onToggleSidebar} className="mr-3 p-2 hover:bg-white/10 rounded">
        <Menu size={20} />
      </button>
      <a href="/" className="flex items-center gap-2 font-bold text-lg tracking-tight text-white">
        ...
        <span className="hidden sm:inline text-white drop-shadow-sm">Acquisition Lifecycle</span>
      </a>
      ...
      {user && (
        <div className="flex items-center gap-3 ml-3">
          ...
          <button onClick={logout} className="p-2 hover:bg-white/10 rounded" title="Logout">
            <LogOut size={16} />
          </button>
        </div>
      )}
